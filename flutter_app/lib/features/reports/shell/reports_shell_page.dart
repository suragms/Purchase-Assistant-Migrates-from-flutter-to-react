...
import '../presentation/reports_full_list_page.dart';
import '../presentation/reports_item_tile.dart';
...
import '../presentation/widgets/reports_breakdown_tab.dart';
import '../presentation/widgets/reports_insights_strip.dart';
import '../presentation/widgets/reports_movement_tab.dart';
...
import '../tabs/reports_activity_tab.dart';
import 'reports_compact_header.dart';
import 'reports_sticky_chrome.dart';
import 'reports_search_overlay.dart';
import 'reports_desktop_scaffold.dart';
...
/// Reports analytics shell — compact header, sticky period/tabs, drill-down.
...
class _ReportsShellPageState extends ConsumerState<ReportsShellPage> {
  ...
  void didChangeDependencies() {
    ...
    if (raw == _syncedUrlTab) return;
    _syncedUrlTab = raw;
    final tab = ReportsBiTabX.fromQuery(raw);
    ...
    final stockSec = GoRouterState.of(context).uri.queryParameters['section'];
    if (stockSec == 'slow' || stockSec == 'dead') {
      ref.read(reportsStockSectionProvider.notifier).state = stockSec!;
    }
  }

  @override
  void didUpdateWidget(covariant ReportsShellPage oldWidget) {
    super.didUpdateWidget(oldWidget);
    // Tab state is local; URL/query rebuilds must not reset via [initialTab].
  }
  ...
  String _searchHintForTab(ReportsBiTab tab) => switch (tab) {
        ...
        ReportsBiTab.stock => 'Search stock…',
  ...
  /// Update address bar for deep links without rebuilding shell state.
  ...
  Widget _buildTabContent({
    ...
    switch (_biTab) {
      ...
      case ReportsBiTab.stock:
        final section = ref.watch(reportsStockSectionProvider);
        return SizedBox(
          height: MediaQuery.sizeOf(context).height * 0.65,
          child: ReportsStockTab(
            initialSection: section,
            onSectionChanged: (s) {
              ref.read(reportsStockSectionProvider.notifier).state = s;
            },
          ),
        );
      case ReportsBiTab.stockMovement:
        return const ReportsActivityTab();
      case ReportsBiTab.categories:
      ...
      case ReportsBiTab.items:
        List<TradeReportItemRow> itemRows;
        if (_packFilter == ReportsPackFilter.all) {
          itemRows =
              sortTradeReportItemsAll(List.of(aggAll.itemsAll), _itemSort);
        } else {
          final raw = switch (_packFilter) {
            ReportsPackFilter.bag => aggList.itemsBag,
            ReportsPackFilter.box => aggList.itemsBox,
          ...
          itemRows = sortTradeReportItemsAll(List.of(raw), _itemSort);
        }
        final q = _debouncedQuery.trim().toLowerCase();
        final filteredItems = q.isEmpty
            ? itemRows
            : itemRows.where((r) => r.name.toLowerCase().contains(q)).toList();
        final itemCap = _visibleCap < filteredItems.length
            ? _visibleCap
            : filteredItems.length;
        return SizedBox(
          height: MediaQuery.sizeOf(context).height * 0.55,
          child: ReportsItemsTab(
            rows: filteredItems.take(itemCap).toList(),
            merged: merged,
            isLoading: showSkeleton,
            hasMore: itemCap < filteredItems.length,
            onLoadMore: itemCap < filteredItems.length
                ? () => setState(() => _visibleCap = filteredItems.length)
                : null,
          ),
        );
      case ReportsBiTab.purchases:
        final qP = _debouncedQuery.trim().toLowerCase();
        final filteredPurchases = qP.isEmpty
            ? merged
            : merged.where((p) {
                final hay = [
                  p.supplierName,
        ...
        return SizedBox(
          ...
          child: ReportsPurchasesTab(
            purchases: filteredPurchases.take(purchaseCap).toList(),
            isLoading: showSkeleton,
            hasMore: purchaseCap < filteredPurchases.length,
            onLoadMore: purchaseCap < filteredPurchases.length
                ? () => setState(() => _visibleCap = filteredPurchases.length)
                : null,
          ),
        );
  ...
  Widget build(BuildContext context) {
    ...
    Future<void> openSearch() async {
      final q = await showReportsSearchOverlay(
        context: context,
        hint: _searchHintForTab(_biTab),
        initialQuery: _debouncedQuery,
      );
      if (q == null || !mounted) return;
      _searchCtl.text = q;
      setState(() {
        _debouncedQuery = q.trim();
        _visibleCap = 40;
      });
    }

    Widget tabBody = _buildTabContent(
      aggList: aggList,
      aggAll: aggAll,
      merged: merged,
      showSkeleton: showSkeleton,
    ...
    if (context.isReportsDesktop) {
      tabBody = ReportsDesktopScaffold(
        selectedTab: _biTab,
        onTabSelected: _selectBiTab,
        onMore: _openMoreTabsSheet,
        body: tabBody,
      );
    } else if (context.isReportsTablet &&
        (_biTab == ReportsBiTab.items || _biTab == ReportsBiTab.purchases)) {
      tabBody = ReportsTabletScaffold(
        list: tabBody,
        detail: Center(
          child: Text(
            _biTab == ReportsBiTab.items
                ? 'Select an item to preview'
                : 'Select a purchase to preview',
    ...
    return PopScope(
      ...
      appBar: ReportsCompactHeader(
        tab: _biTab,
        onBack: () => popShellTabOrGoHome(context, ref, homePath: homePath),
        onSearch: () => unawaited(openSearch()),
        onFilter: () => showReportsFilterSheet(context: context, ref: ref),
        filterCount: filterCount,
        onExportPdf: () => _openReportsPdfSheet(merged),
        onShare: merged.isEmpty
            ? () {}
            : () => unawaited(_shareStatementPdf(merged)),
        onMore: _openMoreTabsSheet,
