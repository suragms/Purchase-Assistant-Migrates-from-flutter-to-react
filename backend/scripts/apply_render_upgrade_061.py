"""Apply Alembic 061 on Render Postgres (harisree-db) when preDeploy is missing.

Auth from .cursor/mcp.json Render token (same as fix_misconfigured_bag_units.py).
"""
...
_SQL = _BACKEND / "sql" / "061_catalog_unit_simplify.sql"
_TARGET = "061_catalog_unit_simplify"
...
def main() -> None:
    ...
    if before and before[0] == _TARGET:
        print("Already at 061 — nothing to do.")
        cur.close()
        conn.close()
        return

    if before and before[0] != "060_stock_list_performance_indexes":
    ...
    conn.commit()
    print("Applied 061_catalog_unit_simplify.sql")
