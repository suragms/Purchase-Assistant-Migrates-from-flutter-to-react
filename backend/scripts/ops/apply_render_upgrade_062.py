"""Apply Alembic 062 on Render Postgres (harisree-db) when preDeploy is missing.

Auth from .cursor/mcp.json Render token (same as apply_render_upgrade_061.py).
"""
...
_SQL = _BACKEND / "sql" / "062_trade_report_indexes.sql"
_TARGET = "062_trade_report_indexes"
_PREVIOUS = "061_catalog_unit_simplify"
...
def main() -> None:
    ...
    if before and before[0] == _TARGET:
        print("Already at 062 — nothing to do.")
        cur.close()
        conn.close()
        return
    ...
    conn.commit()
    print("Applied 062_trade_report_indexes.sql")
