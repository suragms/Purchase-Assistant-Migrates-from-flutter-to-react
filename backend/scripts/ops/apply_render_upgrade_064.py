"""Apply Alembic 064 on Render Postgres (report line indexes).

Auth from .cursor/mcp.json Render token (same as apply_render_upgrade_063.py).
"""
...
_SQL = _BACKEND / "sql" / "064_pg_report_line_indexes.sql"
_TARGET = "064_pg_report_line_indexes"
_PREVIOUS = "063_pg_hot_path_indexes"
...
def main() -> None:
    ...
    if before and before[0] == _TARGET:
        print("Already at 064 — nothing to do.")
        cur.close()
        conn.close()
        return
    ...
    conn.commit()
    print("Applied 064_pg_report_line_indexes.sql")
