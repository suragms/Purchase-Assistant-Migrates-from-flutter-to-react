"""Apply Alembic 063 on Render Postgres (harisree-db) when preDeploy is missing.

Auth from .cursor/mcp.json Render token (same as apply_render_upgrade_062.py).
"""
...
_SQL = _BACKEND / "sql" / "063_pg_hot_path_indexes.sql"
_TARGET = "063_pg_hot_path_indexes"
_PREVIOUS = "062_trade_report_indexes"
...
def main() -> None:
    ...
    if before and before[0] == _TARGET:
        print("Already at 063 — nothing to do.")
        cur.close()
        conn.close()
        return
    ...
    conn.commit()
    print("Applied 063_pg_hot_path_indexes.sql")
