...
const STAFF_BRANCHES = [
  { label: "Home", icon: LuHouse, activeIcon: LuHouse, path: "/staff/home" },
...
function branchIndexForPath(path: string): number {
  for (let i = 0; i < STAFF_BRANCHES.length; i++) {
    if (path.startsWith(STAFF_BRANCHES[i].path)) return i;
  }
  return -1;
}
...
export function StaffAppShell() {
  ...
  const currentPath = currentBranch >= 0 ? STAFF_BRANCHES[currentBranch].path : "/staff/home";
  ...
  const bottomNavItems = STAFF_BRANCHES.map((b) => ({
    icon: <b.icon size={22} />,
    label: b.label,
    value: b.path,
  }));
          ...
          >
            ...
            {railExtended && (
              <div className="px-4 py-3 border-t border-brand-border">
                <p className="text-[11px] font-bold text-text-muted uppercase tracking-wider">Staff</p>
              </div>
            )}
          </nav>
